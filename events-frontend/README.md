...
This is the frontend component of the Events Platform project. This project uses [React](https://react.dev/) as a framework and [Next.js](https://nextjs.org/) (using the **pages router**) as a routing and building solution. [pnpm](https://pnpm.io/) is used for project and dependency management (in other words, use `pnpm add` instead of `npm install` when following any guides online). In addition, [Material UI](https://mui.com/material-ui/) and [Zustand](https://zustand.docs.pmnd.rs/) are preconfigured for styling and global state management, respectively. Finally, [ESLint](https://eslint.org/) and [Prettier](https://prettier.io/) and included for dev tooling.
...
Zustand allows you to manage state that you can access from anywhere in your app (i.e., "global state"). This is useful for things like information about the logged-in user, cookie preferences, configuration settings (light vs. dark color scheme), etc.

- [Zustand Documentation](https://zustand.docs.pmnd.rs/) - official documentation for Zustand
- [Zustand Tutorial](https://zustand.docs.pmnd.rs/guides/tutorial-tic-tac-toe) - step-by-step tutorial for building a Tic Tac Toe game with Zustand
- [Zustand with Typescript](https://zustand.docs.pmnd.rs/guides/beginner-typescript) - instructions for using Typescript (which this project does) with Zustand
- [How to Persist State via Session/LocalStorage](https://zustand.docs.pmnd.rs/integrations/persisting-store-data) - guide to set up automatically saving and loading the state between page loads (useful for storage user credentials)
- [How to Persist State via Cookies](https://github.com/pmndrs/zustand/discussions/1716#discussioncomment-5488321) - same thing as above except using cookies
