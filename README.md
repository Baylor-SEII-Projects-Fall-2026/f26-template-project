...
This is the template project for the Baylor Software Engineering II class project for Fall 2026. There are several parts to this template project:
 - `/events-api` - API/Backend - The server component of the project, using Java Spring as a framework.
 - `/events-frontend` - Web/Frontend - The web component of the project, using React and Material UI as a framework.
...
- [Events Fall 2026](#events-fall-2026)
  - [Table of Contents](#table-of-contents)
  - [Note about IDEs](#note-about-ides)
  - [MySQL Database Setup Instructions](#mysql-database-setup-instructions)
  - [Backend (`/events-api`) Setup Instructions](#backend-events-api-setup-instructions)
  - [Frontend (`/events-frontend`) Setup Instructions](#frontend-events-frontend-setup-instructions)
...
In other words:
- Open the `events-api` folder in whatever Java IDE you want to use (the instructions below use IntelliJ).
- Open the `events-frontend` folder in whatever JavaScript IDE you want to use (the instructions below use VS Code or WebStorm).
...
2. Start the database using Docker Compose in the command-line: `docker compose -f docker/local.docker-compose.yml up -d`
 ...
 - You now have a database running on port `:3307`

## Backend (`/events-api`) Setup Instructions
...
2. Install **Java 25** (OpenJDK):
   ...
   - Feel free to install Java 25 in any other way that makes sense for your local environment (such as if you are running Linux or using a package manager like Homebrew or Chocolatey).
3. Setup the project in IntelliJ IDEA:
   1. Open the project at the `/events-api` directory in IntelliJ. In older versions of IntelliJ, you may need to click "Import from existing sources" instead and choose the `/events-api` directory.
...
5. Start the backend. The easiest way to do this is to open `EventsApplication.java` and click the "play" button on the left.
...
## Frontend (`/events-frontend`) Setup Instructions

[(back to top)](#table-of-contents)

1. Install **Visual Studio Code**: https://code.visualstudio.com/download
2. Install **Node.js** version **24**:
   - Just like the backend, installing a version higher than 24 will likely still work, but since the template project is configured using Node 24, it is recommeneded sticking with that
   ...
   - Otherwise, you can download Node 24 directly from the Node.js website: https://nodejs.org/en/download/
3. Install **pnpm**:
   - **Option 1: Via Corepack**
     - Run `npm install --global corepack@latest`
     - Run `corepack enable pnpm`
   - **Option 2: Other Options**
     - If these commands don't work, you can follow your system's instructions on the pnpm website: https://pnpm.io/installation#using-a-standalone-script
   - Verify pnpm is installed with `pnpm -v`
4. Install project dependencies: `pnpm install`
5. Start the frontend: `pnpm dev`
   - `pnpm dev` starts the project in development mode
   - `pnpm build` builds a production version of the project that can be run with `pnpm start`
...
7. (Optional, but highly recommended) Configure ESLint and Prettier in your IDE. The template project comes pre-configured with ESLint and Prettier; see the [frontend's README](./events-frontend/README.md) for more information about these tools.
   1. **VS Code**
...
7. Setup a GitHub runner. The GitHub runner waits for certain actions to happen in your repository and runs a user-defined set of commands when one occurs. We will be using this runner to rebuild and redeploy our project whenever a commit on the `main` branch occurs. First, we need to link the runner to your repository:
   1. Open an SSH terminal to your VM in GCP by going to the "VM instances" tab, then clicking on the "SSH" button in the "Connect" column of the table.
