...
Much of the details regarding setting up your CI/CD workflow is described in the root folder's README along with comments in the sample files we provide. This file will provide a general overview of the tools that are used throughout the process as well as the role of CI/CD in this project specifically.

GitLab has a great writeup if you want to learn more: [What is CI/CD?](https://about.gitlab.com/topics/ci-cd/).

## Role of CI/CD in THIS Project

One common challenge when it comes to getting your code running in the cloud is figuring out an automatic way to get the latest copy of your code onto the cloud virtual machine without having to manually log-in to the VM every time. This is where CI/CD comes in.

As a reminder, we have three major cloud components here:
- **GitHub**: where your code is *stored* (specifically, the `main` branch)
- **Google Cloud Platform (GCP)**: where your code *runs* (specifically, a Compute Engine VM)
- **Docker Compose**: *how* your code runs (specifically, using a `docker-compose.yml` file to automatically run your frontend, backend, and MySQL database together)

How do we make any updates to the code in GitHub automatically appear on the GCP VM without manually logging in? CI/CD! Specifically, we'll use **GitHub Actions (GHA)** for this project.

GitHub Actions allows us to define **workflows** that are triggered by **events** in our GitHub repository, such as a push to the `main` branch (how convenient!). These workflows can automate tasks like running tests, building Docker images, and deploying code to our GCP VM.

The instructions in the `README.md` file walk you through the following:
- Setting up a GHA runner on your GCP VM - this software will allow your GitHub Actions workflows to run **on your VM**
- Defining a GHA workflow file (`sample-github-actions-ci.yml`) - this is basically a list of commands to run
- Using the prebuilt Docker Compose file (`ci.docker-compose.yml`) - this file defines how your Docker containers are set up and run

With all of that in mind, here's our goal: **when we push to `main`, tell the Docker Compose setup on the GCP VM to update and run the latest version of our code.**

Let's break down how that happens step-by-step:

1. Commit to `main` (such as merging a PR)
2. This triggers the "on push" event and kicks off your GHA workflow
3. GHA will tell your GHA runner (which, as a reminder, is running **ON your GCP VM**) to execute the workflow
4. The GHA workflow then runs the following commands (again, on your GCP VM):
   1. Pull the latest version of the code
   2. Build the frontend and backend Docker images and store those on the VM
   3. Shut down the existing frontend and backend containers
   4. Restart the frontend and backend containers using the NEW versions of the Docker images
5. The updated containers are now running the latest version of your code on your GCP VM

And like magic your latest code is now running on your cloud VM! 
